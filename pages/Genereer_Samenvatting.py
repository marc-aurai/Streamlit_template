...
if check_password():
    ...
    with select3:
        selected_home_injuries = st.multiselect(
            "Selecteer {} blessures: ".format(select_match_injuries["home_team"].values[0]), options=list(select_match_injuries.home_injuries.values)
        )
    with select4:
        selected_away_injuries = st.multiselect(
            "Selecteer {} blessures: ".format(select_match_injuries["away_team"].values[0]), options=list(select_match_injuries.away_injuries.values)
        )
