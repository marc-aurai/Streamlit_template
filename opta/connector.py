...
def refactor_df(df_tournament: pd.DataFrame) -> pd.DataFrame:
    df_tournament['awayContestantCode'] = df_tournament['awayContestantCode'].str.replace('NEC', 'N.E.C.', regex=True)
    df_tournament['homeContestantCode'] = df_tournament['homeContestantCode'].str.replace('NEC', 'N.E.C.', regex=True)
    df_tournament['awayContestantCode'] = df_tournament['awayContestantCode'].str.replace('AJX', 'AJA', regex=True)
    df_tournament['homeContestantCode'] = df_tournament['homeContestantCode'].str.replace('AJX', 'AJA', regex=True)
    return df_tournament

def merge(df_espn: pd.DataFrame, df_tournament: pd.DataFrame) -> pd.DataFrame:
    df_merged = pd.merge(df_espn, df_tournament, how='left', left_on=['date', 'home_abbrev', 'away_abbrev'], right_on=['date', 'homeContestantCode', 'awayContestantCode'])
    df_merged.drop(df_merged.filter(regex="Unname"),axis=1, inplace=True) # Drop unnamed columns
    return df_merged
...
if __name__ == "__main__":
    ...
    df_tournament.to_csv("./opta/data/{}_{}.csv".format(table_kind, competition), sep=";")
